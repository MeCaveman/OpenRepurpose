...
Phase 9 migration group 9 moved `/settings` presentation into `apps/web/src/features/settings/`: the spacious configuration surface now maps setup, account, and workflow configuration to their existing owners, records fixed local-installation policy separately from live status, and states the absence of global preference controls in v0.5. It adds no settings API, form control, inferred system metric, navigator, inspector, or activity pane. Configuration behavior, local deployment assumptions, persistence, and API contracts remain unchanged. This work implements the visual vocabulary already specified above, so the design-system version remains 1.0.
