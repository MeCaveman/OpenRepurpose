...
**Current:** `apps/web/src/components/patterns/` contains the patterns justified by v0.5: `PlatformIdentity`, `ConnectionStatus`, `ConnectionCard`, `SourceStatus`, `WorkflowStatus`, `WorkflowNode`, `WorkflowRoute`, `WorkflowCard`, `JobStatus`, and `ResourceEmptyState`. Its `index.ts` is the public pattern entry point. `platform-metadata.ts` owns the restrained platform display mapping separately from generic connection and route presentation. Migrated features, including the workflow route editor and saved-route list, consume these patterns; remaining route-level page composition stays in `App.tsx` until its migration group.
...
**Current:** `apps/web/src/features/overview/` owns the migrated `/` orientation surface. It links only to existing routes and deliberately introduces no overview data contract, metric, or inferred system state. `apps/web/src/features/setup/` owns the migrated `/setup` readiness presentation and its normalized view contracts. `apps/web/src/features/accounts/` owns the migrated `/accounts` credential, Meta-target, capability, and connected-account presentation. `apps/web/src/features/sources/` owns the migrated `/sources` setup, polling-health, observed-item, lifecycle, and action presentation. `apps/web/src/features/workflows/` owns the migrated `/workflows` route builder, feature-local editor/view contracts, progressive optional-stage controls, saved-route normalization, and page states. `apps/web/src/features/jobs/` owns the migrated `/jobs` execution ledger, compact structured-list adaptation, feature-local job/attempt view contracts, cancellation presentation, destination checkpoints, and contextual attempt-history surface. At compact and narrow widths, selection replaces the ledger with one primary detail pane; wide layouts may show both. `apps/web/src/features/settings/` owns the migrated `/settings` configuration-ownership ledger and fixed local-installation policy. It links to the existing setup, account, and workflow owners and deliberately contains no request, inferred runtime state, or editable global preference. `apps/web/src/features/media/` owns the migrated `/media` local-import form, persisted-file ledger, media state, and YouTube/TikTok direct-publish preparation. Publish preparation stays closed until an asset is selected, sits beside the ledger at wide widths, and becomes the single primary pane at smaller widths. `apps/web/src/features/not-found/` owns the existing unknown-route state and its Dashboard recovery path. These feature components compose existing primitives and domain patterns and contain no request or business logic. `App.tsx` continues to own the unchanged setup, account, source, media, workflow, and job requests and mutations plus shared resource state because those values are consumed across current flows.
