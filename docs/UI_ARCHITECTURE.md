...
The primitive, layout, and domain-pattern layers now exist. Shared navigation presentation lives in the layout layer; `/`, `/setup`, `/accounts`, `/sources`, `/media`, `/workflows`, `/jobs`, `/settings`, and the unknown-route fallback have extracted page bodies below `apps/web/src/features/`. Route-level composition, page metadata, global-navigation configuration/state, shared resource state, and data access remain in `App.tsx`. Future feature directories remain migration targets rather than claims about the existing repository.
