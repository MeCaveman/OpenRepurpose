...
| Documentation feature presentation     | `apps/web/src/features/docs/`               | Local setup handbook composed from the existing setup workspace, panels, alerts, and token-backed links                                   |
...
| Workflows feature presentation         | `apps/web/src/features/workflows/`          | Structured route editor, preset-first transform recipes, route normalization, loading/error/empty states, and saved-workflow presentation |
| Jobs feature presentation              | `apps/web/src/features/jobs/`               | Responsive execution ledger, cancellation state, transform progress/derivative inspection, destination checkpoints, and attempt history   |
...
| Media feature presentation             | `apps/web/src/features/media/`              | Local import, responsive persisted-media ledger, direct-publish preparation, and media-scoped transcript editing/export                   |
| Models feature presentation            | `apps/web/src/features/models/`             | Local whisper.cpp catalog, storage/disk state, explicit downloads, checksum verification, and destructive removal                         |
...
| Global CSS entry                       | `apps/web/src/styles.css`                   | Imports self-hosted font faces, Tailwind, and token layers; applies global focus, selection, font, surface, and reduced-motion behavior   |
...
The current routes are `/`, `/setup`, `/docs`, `/accounts`, `/sources`, `/media`, `/workflows`, `/jobs`, `/models`, and `/settings`, plus the existing unknown-route fallback. `/docs` is a setup-linked local handbook rather than a global resource-rail destination. Routing semantics are owned by `App.tsx`.

The primitive, layout, and domain-pattern layers now exist. Shared navigation presentation lives in the layout layer; `/`, `/setup`, `/accounts`, `/sources`, `/media`, `/workflows`, `/jobs`, `/models`, `/settings`, and the unknown-route fallback have extracted page bodies below `apps/web/src/features/`. Route-level composition, page metadata, and global-navigation configuration/state remain in `App.tsx`; feature requests and mutations remain inside their feature boundaries.
...
**Current:** `apps/web/src/components/ui/` contains the shared primitives justified by implemented surfaces: `Alert`, `Badge`, `Button`, `Checkbox`, `ErrorState`, `FormField`, `Input`, `Panel`, `Progress`, `Select`, `Spinner`, and `Textarea`. `Progress` wraps the native progress element for determinate and indeterminate durable work without introducing feature-local status language. `index.ts` is the public primitive entry point. `control-styles.ts` and `utils.ts` are private implementation helpers. The primitives use native semantics, React, Tailwind CSS, and the canonical OpenRepurpose tokens; no second component or styling framework was added.
...
**Current:** `apps/web/src/features/overview/` owns the `/` orientation surface. It links only to existing routes and deliberately introduces no overview data contract, metric, or inferred system state. `apps/web/src/features/setup/` owns `/setup` readiness presentation, request state, and API translation. `apps/web/src/features/accounts/` owns `/accounts` credential, OAuth, Meta-target, capability, connection state, and API translation. `apps/web/src/features/sources/` owns `/sources` setup, polling-health, observed-item, lifecycle, action state, and API translation. `apps/web/src/features/workflows/` owns `/workflows` request state, API translation, route builder, feature-local editor/view contracts, progressive optional-stage controls, saved-route normalization, and page states. `apps/web/src/features/jobs/` owns `/jobs` list/detail requests, cancellation, compact structured-list adaptation, feature-local job/attempt view contracts, destination checkpoints, and contextual attempt history. At compact and narrow widths, selection replaces the ledger with one primary detail pane; wide layouts may show both. `apps/web/src/features/settings/` owns `/settings` configuration ownership and fixed local-installation policy without a request, inferred runtime state, or editable global preference. `apps/web/src/features/media/` owns `/media` local import, account/capability loading, persisted-file ledger, direct-publish preparation, and API translation. Publish preparation stays closed until an asset is selected, sits beside the ledger at wide widths, and becomes the single primary pane at smaller widths. `apps/web/src/features/not-found/` owns the unknown-route state and its Dashboard recovery path.

`apps/web/src/features/models/` owns `/models` catalog and storage requests, download polling, checksum verification, deletion confirmation, and feature-local model view contracts. It composes the existing workbench primitives and introduces no new visual vocabulary. Download execution remains server-owned after the explicit mutation begins; browser polling only reflects progress.

`apps/web/src/features/docs/` owns the local `/docs` handbook linked from setup. It summarizes safe onboarding, platform limitations, workflows/OBS, backup, automation, and privacy/data removal while the shipped Markdown files remain the complete operator documentation. It adds no independent navigation, token, component family, or visual vocabulary.

The media feature also owns the transcript editor opened from one persisted media item. It keeps timestamp and text drafts local until an explicit save, warns before discarding unsaved edits, and delegates validation, revision conflict handling, persistence, and subtitle serialization to the shared transcript application service through typed HTTP endpoints. Below the wide-workbench boundary, transcript editing replaces the media ledger as the single primary task; at 90rem and above it occupies the existing inspector-width column. This is a feature-local composition of existing primitives and adds no shared visual vocabulary.

`apps/web/src/App.tsx` is the application boundary and route registry. It owns the existing history integration, global navigation, shell composition, page metadata, workspace mode, and unknown-route selection only; it contains no feature request, mutation, or business logic. Route components are exported through each feature's `index.ts`. `apps/web/src/lib/local-api.ts` contains the one shared transport concern currently justified by v0.5: acquiring the local CSRF token. It is not a business-service or global state layer.
...
Additional target directories are created only when a later phase migrates a real component. Do not scaffold empty architecture. Extract along stable boundaries and keep each migration behavior-preserving.
...
The approved primary families are self-hosted through exact Fontsource dependencies: Geist Latin 400/500/600 for interface text and JetBrains Mono Latin 400/600 for technical text. The imported package CSS declares `font-display: swap`, Vite bundles the font assets, and the application makes no runtime font request. Family stacks remain reference tokens; feature code selects technical typography only through `--or-font-technical`.
...
The current direct requests live in typed feature-local route components. `App.tsx` does not own fetched resources or mutations. This architecture does not authorize a new state-management or data-fetching dependency; introduce a major dependency only for a demonstrated need and follow the repository ADR rule.
